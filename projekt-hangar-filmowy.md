...
- **Fonts:** 
  - **Google Fonts** - "Google Sans" (primary), "Inter" (fallback)
  
**Komponenty do stworzenia:**
- `<Navbar />` - fixed header z scroll detection
- `<Hero />` - full-screen z parallax
- `<SectionAbout />` - split layout
- `<FeatureBox />` - reusable dla "Dlaczego My"
- `<ServiceCard />` - dla oferty
- `<TargetCard />` - dla grup docelowych
- `<TimelineStep />` - dla procesu
- `<EventConfigurator />` - **interaktywny konfigurator z formularzem kontaktowym**
  - `<EventTypeButtons />` - 4 przyciski wyboru
  - `<AudienceSlider />` - suwak liczby widzów
  - `<ExtrasToggles />` - przełączniki dodatków
  - `<ContactFormFields />` - pola formularza (Imię, Nazwisko, Email, Telefon, Wiadomość)
- `<ConfigurationSummary />` - **podsumowanie po prawej z przyciskiem submit**
- `<Footer />` - dark theme

### 3.2 Wymagania Wizualne - Szczegóły Implementacji

**Gradient Definicje (z konceptu):**
```css
/* Hero Background */
.hero-gradient {

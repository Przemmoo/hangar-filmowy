...
export async function POST(request: Request, context: RouteParams) {
  try {
    ...
    const submission = await dbSelectOne(
      'SELECT email, firstName, lastName FROM form_submissions WHERE id = ?',
      [submissionId]
    ) as { email: string; firstName: string; lastName: string } | null;
    ...
    // Prepare email HTML with full template
...
<head>
  ...
  <style>
    ...
    .header { background: linear-gradient(135deg, #0A1828, #1E3A5F); padding: 40px 20px; text-align: center; border-radius: 10px 10px 0 0; }
    ...
    .highlight { background: linear-gradient(135deg, #FFD700, #FFA500); color: white; padding: 15px; border-radius: 8px; margin: 15px 0; text-align: center; }
    .kpo-banner { background: linear-gradient(to right, #ffffff, #f9fafb, #ffffff); border: 1px solid #e5e7eb; border-radius: 8px; padding: 20px; margin: 20px 0; text-align: center; }
    .kpo-logos { display: flex; flex-wrap: wrap; justify-content: center; align-items: center; gap: 15px; margin-bottom: 15px; }
    .kpo-logos img { height: 50px; width: auto; }
  </style>
</head>
<body>
  <div class="container">
    ...
    <div class="content">
      ...
      <div class="kpo-banner">
        <a href="https://hangarfilmowy.pl/kpo" style="text-decoration: none; color: inherit; display: block;">
          <div class="kpo-logos">
            <img src="https://hangarfilmowy.pl/kpo-fe-popc.jpg" alt="Fundusze Europejskie" />
            <img src="https://hangarfilmowy.pl/kpo-barwy-rp.jpg" alt="Rzeczypospolita Polska" />
            <img src="https://hangarfilmowy.pl/kpo-kpo.jpg" alt="KPO" />
            <img src="https://hangarfilmowy.pl/kpo-nextgeneU.jpg" alt="Next Generation EU" />
          </div>
